#!/usr/bin/env python3
# Adapted from llama-stack/scripts/openai_coverage.py
# Copyright (c) Meta Platforms, Inc. and affiliates.

"""
OpenAI API Conformance Analyzer for Responses Gateway

Uses oasdiff to compare the gateway's OpenAPI spec against OpenAI's spec
...
def _run_oasdiff(openai_spec: Path, gateway_spec: Path) -> dict[str, Any]:
...
def main():
    parser = argparse.ArgumentParser(description="OpenAI API conformance analyzer for Responses Gateway")
    parser.add_argument(
        "--openai-spec",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Path to Gateway spec (default: docs/openapi.yaml)",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    try:
        report = calculate_coverage(args.openai_spec, args.gateway_spec, args.openresponses_spec)
    except FileNotFoundError:
        print("Error: oasdiff not found. Install with: brew install oasdiff")
        sys.exit(1)
